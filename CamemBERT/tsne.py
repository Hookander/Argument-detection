...
def show_tsne_dom(model, clean_props = True, typ = 'dom', ratio = [0.9, 0.1]):
    sentences, cleaned_labels, domains = get_data_with_simp_labels()

    tokenized_sentences = tokenize_sentences(sentences)

    train_dl, val_dl, test_dl = get_dataloaders(tokenized_sentences, domains, ratio=ratio, batch_size=16)
    ...
    labels = domains
    train_labels = get_labels_from_ratio(labels, ratio)[1]
    if clean_props:
